      ...
      >
        <div className="flex h-full w-full select-none items-center justify-between">
          <div className="flex flex-row items-center gap-x-4">
            <Link href="/">
              <h1 className="neutral-200 mr-5 text-2xl font-bold">Next</h1>
            </Link>
            {session && (
              <Link
                href="/dashboard"
            ...
            <Drawer>
              <DrawerTrigger
                className={
                  session
                    ? ""
                    : "inline-flex items-center justify-center whitespace-nowrap rounded-full bg-violet-800 px-4 py-2 text-sm font-medium outline-0 ring-0 transition-colors hover:bg-violet-700 disabled:pointer-events-none disabled:opacity-50"
                }
              >
              ...
              <DrawerContent>
                <DrawerHeader>
                  <DrawerTitle>
                    {session ? "Next Authentication" : "Welcome to Next"}
                  </DrawerTitle>
                  <DrawerDescription>
                    {session
                      ? `You logged in as ${session?.user?.email}!`
                      : "You can continue access the dashboard by logging in with your github account."}
                  </DrawerDescription>
                </DrawerHeader>
                ...
                <div className="mb-8 flex flex-col gap-y-2 text-center md:hidden">
                  <Separator className="my-2" />
                  {session && (
                    <Link href="/dashboard" className={routeStyle}>
                      Dashboard
                    </Link>
                  )}
                  {Routes.map((route) => (
